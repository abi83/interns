...
workflow steps call these subcommands straight from YAML (`python -m
pipeline.labels set-pr-label ...`), separately from the many other
entrypoints that use these two modules as libraries.
"""
...
def test_labels_issue_labels_prints_a_comma_joined_list(scenario):
    ...
    result = scenario.run("pipeline.labels", REPO, "issue-labels", ISSUE)
...
def test_labels_pr_labels_prints_a_comma_joined_list(scenario):
    ...
    result = scenario.run("pipeline.labels", REPO, "pr-labels", PR)
...
def test_labels_set_issue_status_moves_between_statuses(scenario):
    ...
    result = scenario.run("pipeline.labels", REPO, "set-issue-status", ISSUE, "status:ready")
...
def test_labels_edit_issue_labels_only_changes_what_is_missing(scenario):
    ...
    result = scenario.run("pipeline.labels", REPO, "edit-issue-labels", ISSUE,
...
def test_labels_set_pr_label_switches_the_pipeline_label(scenario):
    ...
    result = scenario.run("pipeline.labels", REPO, "set-pr-label", PR, "pr:in-review")
...
def test_labels_set_pr_label_with_no_target_clears_all_pipeline_labels(scenario):
    ...
    result = scenario.run("pipeline.labels", REPO, "set-pr-label", PR)
...
def test_labels_escalate_pr_sets_needs_attention(scenario):
    ...
    result = scenario.run("pipeline.labels", REPO, "escalate-pr", PR)
...
def test_verdict_verdict_for_head_prints_the_latest_state(scenario):
    ...
    result = scenario.run("pipeline.verdict", REPO, PR, "reviewer-bot", "verdict-for-head", HEAD)
...
def test_verdict_rounds_requested_excludes_the_given_commit(scenario):
    ...
    result = scenario.run("pipeline.verdict", REPO, PR, "reviewer-bot", "rounds-requested", "--exclude-commit", HEAD)
...
def test_verdict_review_count_only_counts_the_given_login(scenario):
    ...
    result = scenario.run("pipeline.verdict", REPO, PR, "reviewer-bot", "review-count")
...
def test_verdict_summary_for_head_prints_both_verdict_and_count_from_one_fetch(scenario):
    ...
    result = scenario.run("pipeline.verdict", REPO, PR, "reviewer-bot", "summary-for-head", HEAD)
