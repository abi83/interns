...
from interns.steps.push import pr_open_for_issue, push_branch
...
@patch("interns.steps.push.default_branch", return_value="main")
def test_push_branch_squashes_and_pushes(_):
    ...
    with patch("interns.steps.push.subprocess.run", side_effect=seq):
        result = push_branch(REPO, WORKSPACE)

    assert "feat/my-branch" in result


@patch("interns.steps.push.default_branch", return_value="develop")
def test_push_branch_uses_repos_default_branch(_):
    seq = _git_seq("feat/x", "", "abc123", "def456", "msg", "", "", "file.py", "")
    with patch("interns.steps.push.subprocess.run", side_effect=seq) as run:
...
def test_push_branch_rejects_main(_):
    with patch("interns.steps.push.subprocess.run", return_value=_proc("main")):
...
def test_push_branch_rejects_master(_):
    with patch("interns.steps.push.subprocess.run", return_value=_proc("master")):
        with pytest.raises(PushRefusedError, match="Refusing"):
            push_branch(REPO, WORKSPACE)
...
@patch("interns.steps.push.default_branch", return_value="main")
@patch("interns.steps.push.pr_create", return_value="https://github.com/owner/repo/pull/1")
def test_pr_open_for_issue_appends_closes(mock_create, _):
    with patch("interns.steps.push.subprocess.run", return_value=_proc("feat/x")):
...
def test_pr_open_for_issue_rejects_detached_head():
    with patch("interns.steps.push.subprocess.run", return_value=_proc("HEAD")):
