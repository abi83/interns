...
from interns.gh import transport as gh_transport
...
class RunTests(unittest.TestCase):
    def test_gh_not_installed(self):
        with patch("interns.gh.transport.subprocess.run", side_effect=FileNotFoundError()):
    ...
    def test_command_failure_raises_with_stderr(self):
        ...
        with patch("interns.gh.transport.subprocess.run", side_effect=exc):
            with self.assertRaisesRegex(gh.GhCommandError, "not found"):
                gh.run(["issue", "view", "1"])

    def test_success_returns_completed_process(self):
        with patch("interns.gh.transport.subprocess.run", return_value=_proc(stdout="ok")) as mock_run:
...
class ApiTests(unittest.TestCase):
    def test_get_parses_json_body(self):
        with patch("interns.gh.transport.subprocess.run", return_value=_proc(stdout='{"a": 1}')) as mock_run:
    ...
    def test_empty_body_returns_none(self):
        with patch("interns.gh.transport.subprocess.run", return_value=_proc(stdout="")):
            self.assertIsNone(gh.api("repos/acme/widgets"))

    def test_post_sends_fields(self):
        with patch("interns.gh.transport.subprocess.run", return_value=_proc(stdout="")) as mock_run:
...
class ApiStatusTests(unittest.TestCase):
    def test_ok(self):
        with patch("interns.gh.transport.subprocess.run", return_value=_proc(stdout='{"a": 1}')):
...
class IssueViewEditTests(unittest.TestCase):
    def test_view_requests_only_given_fields(self):
        with patch("interns.gh.transport.subprocess.run", return_value=_proc(stdout='{"labels": []}')) as mock_run:
    ...
    def test_edit_is_noop_with_nothing_to_change(self):
        with patch("interns.gh.transport.subprocess.run") as mock_run:
            gh.issue_edit("acme/widgets", 42)
        mock_run.assert_not_called()

    def test_edit_adds_and_removes_labels(self):
        with patch("interns.gh.transport.subprocess.run", return_value=_proc()) as mock_run:
...
class PrViewEditCreateTests(unittest.TestCase):
    def test_view_requests_only_given_fields(self):
        with patch("interns.gh.transport.subprocess.run", return_value=_proc(stdout='{"state": "OPEN"}')) as mock_run:
    ...
    def test_list_requests_given_fields_and_state(self):
        with patch("interns.gh.transport.subprocess.run", return_value=_proc(stdout='[{"number": 1}]')) as mock_run:
    ...
    def test_list_raises_when_truncated(self):
        ...
        with patch("interns.gh.transport.subprocess.run", return_value=_proc(stdout=prs)):
    ...
    def test_create_returns_url(self):
        ...
        with patch("interns.gh.transport.subprocess.run", return_value=_proc(stdout=f"{url}\n")) as mock_run:
...
class PrDiffNamesTests(unittest.TestCase):
    def test_lists_changed_files(self):
        with patch("interns.gh.transport.subprocess.run", return_value=_proc(stdout="a.ts\nb.ts\n")) as mock_run:
...
class PrChecksTests(unittest.TestCase):
    def test_returns_the_parsed_checks(self):
        ...
        with patch("interns.gh.transport.subprocess.run", return_value=_proc(stdout=json.dumps(checks))) as mock_run:
    ...
    def test_returns_checks_despite_nonzero_exit(self):
        ...
        with patch("interns.gh.transport.subprocess.run", return_value=_proc(stdout=json.dumps(checks), returncode=1)):
            self.assertEqual(gh.pr_checks("acme/widgets", 5), checks)

    def test_empty_list_when_no_checks_reported(self):
        proc = _proc(stdout="", returncode=1, stderr="no checks reported on the 'x' branch")
        with patch("interns.gh.transport.subprocess.run", return_value=proc):
            self.assertEqual(gh.pr_checks("acme/widgets", 5), [])

    def test_raises_on_other_failure(self):
        with patch("interns.gh.transport.subprocess.run", return_value=_proc(stdout="", returncode=1, stderr="HTTP 502")):
            with self.assertRaises(gh.GhCommandError):
                gh.pr_checks("acme/widgets", 5)

    def test_raises_on_malformed_output(self):
        with patch("interns.gh.transport.subprocess.run", return_value=_proc(stdout="not json")):
            with self.assertRaises(gh.GhCommandError):
                gh.pr_checks("acme/widgets", 5)
...
class ApiAllPagesTests(unittest.TestCase):
    def test_flattens_every_page(self):
        with patch("interns.gh.transport.subprocess.run", return_value=_proc(stdout='[[1, 2], [3]]')) as mock_run:
...
class ListSecretNamesTests(unittest.TestCase):
    def test_lists_names_across_pages(self):
        with patch("interns.gh.transport.subprocess.run", return_value=_proc(stdout="A\nB\n")):
...
class DefaultBranchTests(unittest.TestCase):
    def test_reads_default_branch(self):
        with patch("interns.gh.transport.subprocess.run", return_value=_proc(stdout='{"default_branch": "trunk"}')):
...
class SharedClientAdditionsTests(unittest.TestCase):
    ...
    def test_label_names(self):
        with patch("interns.gh.ops.label_list", return_value=[{"name": "bug"}, {"name": "x"}]):
