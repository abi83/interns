...
class GhQueryTests(unittest.TestCase):
    def test_head_ref_and_sha(self):
        with patch("interns.gh.query.gh.pr_view", return_value={"headRefName": "b", "headRefOid": "abc"}) as view:
    ...
    def test_closing_issue_first_or_empty(self):
        with patch("interns.gh.query.gh.pr_view", return_value={"closingIssuesReferences": [{"number": 9}, {"number": 10}]}):
            self.assertEqual(gh_query.closing_issue("a/b", 3), "9")
        with patch("interns.gh.query.gh.pr_view", return_value={"closingIssuesReferences": []}):
    ...
    def test_pr_for_issue_matches_closing_reference(self):
        ...
        with patch("interns.gh.query.gh.pr_list", return_value=prs):
    ...
    def test_main_prints_output_lines(self):
        ...
        out = io.StringIO()
        with patch("interns.gh.query.gh.pr_list", return_value=prs), redirect_stdout(out):
