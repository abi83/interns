...
from interns import cli, gh
from interns.steps import safety_checks
...
class CheckSecretsTests(unittest.TestCase):
    def test_passes_when_all_present(self):
        with patch("interns.steps.safety_checks.gh.list_secret_names", return_value=SECRETS):
            self.assertEqual(safety_checks.check_secrets("acme/widgets", SECRETS), [])

    def test_names_a_missing_secret(self):
        with patch("interns.steps.safety_checks.gh.list_secret_names", return_value=[SECRETS[0]]):
    ...
    def test_warns_but_does_not_fail_when_secrets_cant_be_listed(self):
        with patch("interns.steps.safety_checks.gh.list_secret_names", side_effect=gh.GhCommandError("403")), \
...
class CheckVarsTests(unittest.TestCase):
    def test_names_a_missing_variable(self):
        with patch("interns.steps.safety_checks.gh.list_variable_names", return_value=[VARS[0]]):
    ...
    def test_warns_but_does_not_fail_when_variables_cant_be_listed(self):
        with patch("interns.steps.safety_checks.gh.list_variable_names", side_effect=gh.GhCommandError("403")), \
             patch("sys.stderr", io.StringIO()):
...
class MainTests(unittest.TestCase):
    ...
    def test_reports_every_failure_not_just_the_first(self):
        with patch("interns.steps.safety_checks.gh.list_secret_names", return_value=[]), \
             patch("interns.steps.safety_checks.gh.list_variable_names", return_value=[]):
            status = safety_checks._main(self._ctx(), [])
        self.assertEqual(status, 1)

    def test_all_pass_returns_zero(self):
        with patch("interns.steps.safety_checks.gh.list_secret_names", return_value=SECRETS), \
             patch("interns.steps.safety_checks.gh.list_variable_names", return_value=VARS):
            status = safety_checks._main(self._ctx(), [])
    ...
    def test_fails_when_gh_token_unset(self):
        ...
        with patch.dict(os.environ, {"GITHUB_REPOSITORY": "acme/widgets"}, clear=True):
            with self.assertRaises(cli.MissingEnvError):
                from interns.ctx import ActionsCtx as _Ctx
                _Ctx.from_env()
