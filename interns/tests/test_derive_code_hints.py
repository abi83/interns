...
from interns.steps import derive_code_hints
...
class DeriveCodeHintsTests(unittest.TestCase):
    def test_type_coding_task_maps_to_feat_and_a_feat_branch(self):
        with patch("interns.steps.derive_code_hints.labels.issue_labels", return_value=["type:coding-task", "status:ready"]), \
             patch("interns.steps.derive_code_hints.gh.issue_view", return_value={"title": "Pass the coder deterministic commit-type hints"}):
    ...
    def test_type_bug_maps_to_fix_and_a_fix_branch(self):
        with patch("interns.steps.derive_code_hints.labels.issue_labels", return_value=["type:bug", "status:ready"]), \
             patch("interns.steps.derive_code_hints.gh.issue_view", return_value={"title": "Crash on empty input"}):
            hint, branch = derive_code_hints.derive_code_hints("acme/widgets", 42)
    ...
    def test_slug_strips_punctuation_collapses_separators_trims_trailing_dashes(self):
        with patch("interns.steps.derive_code_hints.labels.issue_labels", return_value=["type:coding-task"]), \
             patch("interns.steps.derive_code_hints.gh.issue_view", return_value={"title": "  Refine: the (refiner) -- stop!  "}):
    ...
    def test_raises_when_no_implementable_type_label_is_present(self):
        with patch("interns.steps.derive_code_hints.labels.issue_labels", return_value=["type:spike"]):
...
class CliTests(unittest.TestCase):
    ...
    def test_writes_the_hints_to_github_output(self):
        with patch("interns.steps.derive_code_hints.derive_code_hints", return_value=("feat:", "feat/issue-42-x")), \
