...
def _yaml(data: object, *, exists: bool = True):
    ...
    return (
        patch("interns.config.os.path.isfile", return_value=exists),
        patch("interns.config._yaml_to_json", return_value=data),
    )
...
class LoadRawTests(unittest.TestCase):
    ...
    def test_yq_failure_is_surfaced(self):
        isfile_p = patch("interns.config.os.path.isfile", return_value=True)
        run_p = patch("interns.config.subprocess.run", return_value=_cp(returncode=1, stderr="bad yaml"))
        with isfile_p, run_p:
    ...
    def test_missing_yq_binary_is_surfaced(self):
        isfile_p = patch("interns.config.os.path.isfile", return_value=True)
        run_p = patch("interns.config.subprocess.run", side_effect=FileNotFoundError())
        with isfile_p, run_p:
...
class CliTests(unittest.TestCase):
    ...
    def test_config_error_exits_non_zero(self):
        with patch("interns.config.load_raw", side_effect=config.ConfigError("boom")):
