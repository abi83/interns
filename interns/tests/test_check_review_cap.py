...
from interns import check_review_cap
...
class CheckCapTests(unittest.TestCase):
    def test_under_the_cap_is_a_no_op(self):
        ...
        with patch("interns.check_review_cap.verdict.reviews_by", return_value=reviews), \
             patch("interns.check_review_cap.labels.escalate_pr") as escalate, \
             patch("interns.check_review_cap.gh.pr_comment") as comment:
    ...
    def test_over_the_cap_also_caps(self):
        ...
        with patch("interns.check_review_cap.verdict.reviews_by", return_value=reviews), \
             patch("interns.check_review_cap.labels.escalate_pr"), \
             patch("interns.check_review_cap.gh.pr_comment"):
            self.assertTrue(check_review_cap.check_cap("acme/widgets", 12, "reviewer[bot]", 5))

    def test_a_given_count_skips_the_fetch(self):
        with patch("interns.check_review_cap.verdict.reviews_by") as reviews_by, \
             patch("interns.check_review_cap.labels.escalate_pr") as escalate, \
             patch("interns.check_review_cap.gh.pr_comment"):
    ...
    def test_writes_capped_output_using_ctx(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.dict(os.environ, {"GITHUB_OUTPUT": output_file}), \
                 patch("interns.check_review_cap.check_cap", return_value=True) as fn:
