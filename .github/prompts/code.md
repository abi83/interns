...
the repo has no `Makefile`, testing isn't configured for it yet — skip this
step rather than inventing a test command yourself. The reviewer does not
run tests — the CI checks are the gate, and a red check sends the issue
straight to a human instead of back to you. Do not open the PR with a
failing build or failing tests: if you can't get them green, stop and
