...
from pipeline import apply_verdict
...
class ApplyVerdictTests(unittest.TestCase):
    def test_approved_clears_the_pr_label_and_tells_the_issue(self):
        with patch("pipeline.apply_verdict.gh.pr_view", return_value={"headRefOid": "headsha"}), \
             patch("pipeline.apply_verdict.gh.pr_url", return_value="https://github.com/acme/widgets/pull/12"), \
             patch("pipeline.apply_verdict.gh.run_url", return_value="https://x/runs/1"), \
             ...
             patch("pipeline.apply_verdict.labels.set_pr_pipeline_label") as set_pr, \
             patch("pipeline.apply_verdict.gh.issue_comment") as comment:
    ...
    def test_first_changes_requested_with_linked_issue_dispatches_a_fix_round(self):
        with patch("pipeline.apply_verdict.gh.pr_view", return_value={"headRefOid": "headsha"}), \
             ...
             patch("pipeline.apply_verdict.labels.set_pr_pipeline_label") as set_pr, \
             patch("pipeline.apply_verdict.gh.dispatch_workflow") as dispatch:
            apply_verdict.apply_verdict("acme/widgets", 12, 34, "reviewer[bot]")
    ...
    def test_second_changes_requested_escalates_without_dispatching(self):
        with patch("pipeline.apply_verdict.gh.pr_view", return_value={"headRefOid": "headsha"}), \
             ...
             patch("pipeline.apply_verdict.labels.escalate_pr") as escalate, \
             patch("pipeline.apply_verdict.labels.set_issue_status") as set_issue, \
             patch("pipeline.apply_verdict.gh.pr_comment") as comment, \
             patch("pipeline.apply_verdict.gh.dispatch_workflow") as dispatch:
            apply_verdict.apply_verdict("acme/widgets", 12, 34, "reviewer[bot]")
    ...
    def test_stale_review_against_an_earlier_commit_flags_for_a_human(self):
        with patch("pipeline.apply_verdict.gh.pr_view", return_value={"headRefOid": "headsha"}), \
             ...
             patch("pipeline.apply_verdict.labels.escalate_pr"), \
             patch("pipeline.apply_verdict.labels.set_issue_status"), \
             patch("pipeline.apply_verdict.gh.pr_comment") as comment:
            apply_verdict.apply_verdict("acme/widgets", 12, 34, "reviewer[bot]")
        self.assertIn("without submitting a recognized verdict", comment.call_args[0][2])
