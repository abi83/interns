...
class BuildFeedbackTextTests(unittest.TestCase):
    def test_builds_the_review_comments_and_conversation_block(self):
        review = {"id": 123, "submitted_at": "2026-01-02T03:04:05Z", "body": "do the thing", "state": "CHANGES_REQUESTED"}
        ...
        with patch("pipeline.gather_fix_feedback.gh.api_all_pages", side_effect=fake_pages):
    ...
    def test_raises_when_there_is_no_changes_requested_review(self):
        with patch("pipeline.gather_fix_feedback.gh.api_all_pages", return_value=[]):
...
class CliTests(unittest.TestCase):
    ...
    def test_writes_the_feedback_block_to_github_output(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue(out.startswith("text<<EOF_FEEDBACK\n"))
            self.assertTrue(out.endswith("EOF_FEEDBACK\n"))
