...
def ensure_available() -> None:
    if shutil.which("gh") is None:
        raise gh_transport.GhNotInstalledError("the `gh` CLI is not installed or not on PATH")
    gh_transport.run(["auth", "status"])
...
def auth_scopes() -> set[str]:
    ...
    proc = gh_transport.run(["auth", "status"], check=False)
...
def current_repo(explicit: str | None) -> Repo:
    ...
    data = json.loads(gh_transport.run(args).stdout)
    owner = data["owner"]["login"]
    name = data["name"]
    # `repo view --json owner` carries only id and login, not the owner type.
    owner_type = gh_transport.api(f"repos/{owner}/{name}")["owner"]["type"]
...
def set_secret(repo: str, name: str, value: str) -> None:
    ...
    gh_transport.run(["secret", "set", name, "--repo", repo], input_text=value)
...
def set_variable(repo: str, name: str, value: str) -> None:
    ...
    try:
        gh_transport.run(["variable", "delete", name, "--repo", repo])
    except gh_transport.GhCommandError as exc:
        if "HTTP 404" not in str(exc):
            raise
    gh_transport.run(["variable", "set", name, "--repo", repo], input_text=value)


def get_file(repo: str, path: str, ref: str) -> str:
    """Text content of `path` in `repo` at `ref`, via the contents API."""
    data = gh_transport.api(f"repos/{repo}/contents/{path}?ref={ref}")
    if not isinstance(data, dict) or "content" not in data:
        raise gh_transport.GhCommandError(f"could not read {path} from {repo}@{ref}")
    return base64.b64decode(data["content"]).decode()


def path_exists(repo: str, path: str, ref: str) -> bool:
    """True when `path` (a file or directory) is present in `repo` at `ref`."""
    status, _ = gh_transport.api_status(f"repos/{repo}/contents/{path}?ref={ref}")
    if status == "blocked":
        raise gh_transport.GhCommandError(f"could not check {path} in {repo}@{ref}")
    return status == "ok"
...
def list_dir(repo: str, path: str, ref: str) -> list[str]:
    ...
    if not isinstance(data, list):
        raise gh_transport.GhCommandError(f"{path} in {repo}@{ref} is not a directory")
    return [entry["name"] for entry in data]


def branch_head_sha(repo: str, branch: str) -> str:
    data = gh_transport.api(f"repos/{repo}/git/ref/heads/{branch}")
    if not isinstance(data, dict):
        raise gh_transport.GhCommandError(f"could not resolve heads/{branch} on {repo}")
    return data["object"]["sha"]


def ref_exists(repo: str, branch: str) -> bool:
    status, _ = gh_transport.api_status(f"repos/{repo}/git/ref/heads/{branch}")
    if status == "blocked":
        raise gh_transport.GhCommandError(f"could not check heads/{branch} on {repo}")
    return status == "ok"


def create_branch(repo: str, branch: str, sha: str) -> None:
    gh_transport.api(f"repos/{repo}/git/refs", method="POST",
        fields={"ref": f"refs/heads/{branch}", "sha": sha})


def create_blob(repo: str, content: str) -> str:
    data = gh_transport.api(f"repos/{repo}/git/blobs", method="POST",
               fields={"content": content, "encoding": "utf-8"})
    if not isinstance(data, dict):
        raise gh_transport.GhCommandError(f"could not create blob in {repo}")
    return data["sha"]


def create_tree(repo: str, entries: list[dict]) -> str:
    data = gh_transport.api(f"repos/{repo}/git/trees", method="POST",
               input_json=json.dumps({"tree": entries}))
    if not isinstance(data, dict):
        raise gh_transport.GhCommandError(f"could not create tree in {repo}")
    return data["sha"]


def create_commit(repo: str, message: str, tree: str, parents: list[str]) -> str:
    data = gh_transport.api(f"repos/{repo}/git/commits", method="POST",
               input_json=json.dumps({"message": message, "tree": tree, "parents": parents}))
    if not isinstance(data, dict):
        raise gh_transport.GhCommandError(f"could not create commit in {repo}")
    return data["sha"]


def get_existing_file(repo: str, path: str, ref: str) -> tuple[str, str] | None:
    """(content, sha) for `path` in `repo` at `ref`, or None if absent."""
    status, data = gh_transport.api_status(f"repos/{repo}/contents/{path}?ref={ref}")
    if status == "missing":
        return None
...
def put_file(repo: str, path: str, content: str, message: str, branch: str,
    ...
    if sha is not None:
        fields["sha"] = sha
    gh_transport.api(f"repos/{repo}/contents/{path}", method="PUT", fields=fields)


def branch_protection_state(repo: str, branch: str) -> tuple[str, object]:
    """`api_status` of the branch's protection: ("ok", body), ("missing", None)
    or ("blocked", None)."""
    return gh_transport.api_status(f"repos/{repo}/branches/{branch}/protection")


def set_branch_protection(repo: str, branch: str, protection: dict) -> None:
    gh_transport.api(f"repos/{repo}/branches/{branch}/protection",
           method="PUT", input_json=json.dumps(protection))


def pages_state(repo: str) -> tuple[str, object]:
    """`api_status` of the repo's Pages site."""
