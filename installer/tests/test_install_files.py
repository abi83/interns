...
class CollectMissingFilesTests(unittest.TestCase):
    def test_collect_only_missing_files(self):
        ...
        with mock.patch.object(install_files.gh, "path_exists",
                               side_effect=lambda r, p, ref: p in present), \
             mock.patch.object(install_files.gh, "get_existing_file", side_effect=existing), \
             mock.patch.object(install_files.gh, "get_file", side_effect=lambda r, p, ref: f"body:{p}"):
    ...
    def test_collect_flags_drifted_wrapper_file(self):
        ...
        with mock.patch.object(install_files.gh, "path_exists", return_value=True), \
             mock.patch.object(install_files.gh, "get_existing_file", side_effect=existing), \
             mock.patch.object(install_files.gh, "get_file", side_effect=lambda r, p, ref: f"body:{p}"):
            wanted = collect_missing_files(_repo(), "main", issue_templates=False)
    ...
    def test_collect_never_overwrites_existing_config_files(self):
        ...
        with mock.patch.object(install_files.gh, "path_exists", return_value=True), \
             mock.patch.object(install_files.gh, "get_existing_file",
    ...
    def test_collect_substitutes_ref_placeholder(self):
        ...
        with mock.patch.object(install_files.gh, "path_exists", return_value=False), \
             mock.patch.object(install_files.gh, "get_existing_file", return_value=None), \
             mock.patch.object(install_files.gh, "get_file", return_value=tmpl), \
    ...
    def test_collect_pulls_issue_templates_when_dir_absent(self):
        with mock.patch.object(install_files.gh, "path_exists", return_value=False), \
             mock.patch.object(install_files.gh, "get_existing_file", return_value=None), \
             mock.patch.object(install_files.gh, "list_dir", return_value=["bug.md", "config.yml"]), \
             mock.patch.object(install_files.gh, "get_file", side_effect=lambda r, p, ref: f"body:{p}"):
